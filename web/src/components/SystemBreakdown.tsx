...
export default function SystemBreakdown() {
  return (
    <div className="divide-y divide-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950">
      {ROWS.map((row) => (
        <div key={row.name} className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center">
          <div className="flex shrink-0 flex-col items-center gap-2 sm:w-32">
            <div className="flex h-12 w-12 items-center justify-center rounded-full border border-zinc-300 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300">
          ...
          <div className="flex-1">
            <div className="flex flex-wrap gap-2">
              {row.tags.map((tag) => (
                <span
                  key={tag}
                  className="rounded-full border border-zinc-300 dark:border-zinc-700 px-2.5 py-0.5 font-mono text-xs text-zinc-700 dark:text-zinc-300"
                >
                  {tag}
                </span>
              ))}
            </div>
            <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">{row.body}</p>
          </div>
        </div>
      ))}
    </div>
