...
function Bar({
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="w-16 shrink-0 font-mono text-xs text-zinc-400">{label}</span>
      ...
      <span className="w-28 shrink-0 text-right font-mono text-xs text-zinc-300">
...
function MetricPanel({
  ...
  return (
    <div className="space-y-2">
      ...
      <p className={`text-right font-mono text-xs ${isBetter ? "text-emerald-400" : "text-amber-400"}`}>
...
export default function SchedulerCompare() {
  ...
  if (error) {
    return (
      <div className="w-full max-w-xl rounded-lg border border-zinc-800 bg-zinc-950 p-6 text-center text-sm text-zinc-500">
